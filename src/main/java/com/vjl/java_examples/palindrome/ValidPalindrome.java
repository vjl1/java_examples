...
public class ValidPalindrome {
  ...
  public boolean isPalindromeCompareCharAndDigitWithOneSkip(String input) {
    ...
    int skipCount = 0;
    while(start<end){
      while(!Character.isLetterOrDigit(input.charAt(start))) start++;
      while(!Character.isLetterOrDigit(input.charAt(end))) end--;
      if(Character.toLowerCase(input.charAt(start)) != Character.toLowerCase(input.charAt(end))){
        if(skipCount<1){
          skipCount++;
          if(Character.toLowerCase(input.charAt(start+1)) == Character.toLowerCase(input.charAt(end))){
            start++;
          }else if(Character.toLowerCase(input.charAt(start)) == Character.toLowerCase(input.charAt(end-1))){
            end--;
          }else{
            return false;
          }
